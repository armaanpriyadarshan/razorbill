...
| `silence_stop_minutes` | `10.0` | Live mode only: end the meeting after this long with no speech in the live transcript, with an actionable warning notification at 3 minutes. Covers meeting apps that hold the microphone after a call (a rejoin page left open). After a silence stop, no new recording starts until the mic is released once. `0` disables. |
