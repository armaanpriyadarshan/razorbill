...
| `farewell_stop` | `true` | Live mode only: when goodbyes are heard (or a lull follows speech), a small model call (`insight_model`) judges whether the meeting actually ended. Yes arms a 45 second hold, announced by a notification; any new speech cancels it. |
| `silence_stop_minutes` | `10.0` | Backstop: end the meeting after this long with no speech in the live transcript, with an actionable warning notification at 3 minutes. Covers meeting apps that hold the microphone after a call (a rejoin page left open). After either kind of stop, no new recording starts until the mic is released once. `0` disables. |
