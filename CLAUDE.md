...
- `razorbill status --json` prints daemon state; `start`/`stop`/`toggle`
  control it non-interactively; `trash` discards the recording in
  progress; `ask "..."` answers one question over the live transcript or
  latest note; `last` prints the newest note path.
